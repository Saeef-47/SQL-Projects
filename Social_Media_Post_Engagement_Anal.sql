...
-- Count of Likes, Comments, and Shares for a Specific Post ID
SELECT 
    p.post_id,
    p.post_content,
...
-- Mean Number of Reactions per Distinct User by Reaction Date
SELECT
    DATE(reaction_date) AS reaction_day,
    COUNT(DISTINCT user_id) AS distinct_users,
    COUNT(*) AS total_reactions,
    ROUND(COUNT(*)::DECIMAL / COUNT(DISTINCT user_id), 4) AS avg_reactions_per_user
FROM UserReactions
GROUP BY DATE(reaction_date)
ORDER BY reaction_day;

-- Top 3 Most Engaging Posts (by Reactions) Within the Last 7 Days
SELECT
    p.post_id,
    p.post_content,
